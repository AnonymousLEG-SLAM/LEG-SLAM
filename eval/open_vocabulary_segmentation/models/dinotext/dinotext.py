...
from eval.open_vocabulary_segmentation.model import ProjectionLayer, VisualProjectionLayer, CLIPLastLayer, DoubleMLP
from eval.open_vocabulary_segmentation.loss import Contrastive
from eval.open_vocabulary_segmentation.hooks import average_text_tokens, get_vit_out, feats
...
class DINOText(nn.Module):
    ...
    def __init__(
        ...
        with open(os.path.join('eval/open_vocabulary_segmentation/configs', f"{proj_class}.yaml"), 'r') as config_file:
            config = yaml.safe_load(config_file)['model']
            
        ProjClass = getattr(importlib.import_module('eval.open_vocabulary_segmentation.model'), proj_model)
