...
void GaussianTrainer::trainingOnce(
    ...
    torch::Tensor background = torch::tensor(bg_color,
...

        {
            ...
            if (iteration < opt.densify_until_iter_) {
                ...
                if ((iteration > opt.densify_from_iter_) && (iteration % opt.densification_interval_ == 0)) {
                    ...
                    gaussians->densifyAndPrune(opt.densify_grad_threshold_, 0.005, scene->cameras_extent_,
                                               size_threshold);
                }

                if ((iteration % opt.opacity_reset_interval_) == 0 ||
                    (dataset.white_background_ && iteration == opt.densify_from_iter_))
