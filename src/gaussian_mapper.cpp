...
void GaussianMapper::run() {
        ...
        if (hasMetInitialMappingConditions()) {
            ...
            std::vector < ORB_SLAM3::MapPoint * > vpMPs;
            {
                ...
                for (const auto &pMP: vpMPs) {
                    ...
                    // TODO: initialize lang_features_ from the projection of a point onto a lf frame
                    scene_->cachePoint3D(pMP->mnId, point3D);
                }
                for (const auto &pKF: vpKFs) {
                    std::shared_ptr <GaussianKeyframe> new_kf = std::make_shared<GaussianKeyframe>(pKF->mnId,
                                                                                                   getIteration());
                    new_kf->zfar_ = z_far_;
                    ...
                    cv::Mat imgRGB_undistorted, imgAux_undistorted, imgAuxLF_undistorted;
                    try {
                        // Camera
                        ...
                        // Language Features Auxiliary Image
                        imgAuxLF_undistorted = pKF->imgAuxiliaryLF;
                        ...
                        new_kf->language_features_ =
                                tensor_utils::cvMat2TorchTensor_Float32(imgAuxLF_undistorted, device_type_);
                    }
                    ...
                    new_kf->img_auxiliary_lf_undist_ = imgAuxLF_undistorted;
                }
            }
            ...
            for (auto &kfit: scene_->keyframes()) {
                ...
                if (device_type_ == torch::kCUDA) {
                    cv::cuda::GpuMat img_gpu, depth_gpu;
                    ...
                    pkf->gaus_pyramid_original_depth_.resize(num_gaus_pyramid_sub_levels_);
                    for (int l = 0; l < num_gaus_pyramid_sub_levels_; ++l) {
                        cv::cuda::GpuMat img_resized, depth_resized;
                        ...
                        cv::cuda::resize(depth_gpu, depth_resized,
                                         cv::Size(pkf->gaus_pyramid_width_[l], pkf->gaus_pyramid_height_[l]));
                        pkf->gaus_pyramid_original_depth_[l] =
                    ...
                    pkf->gaus_pyramid_original_depth_.resize(num_gaus_pyramid_sub_levels_);
                    for (int l = 0; l < num_gaus_pyramid_sub_levels_; ++l) {
                        cv::Mat img_resized, depth_resized;
                        cv::resize(pkf->img_undist_, img_resized,
                                   cv::Size(pkf->gaus_pyramid_width_[l], pkf->gaus_pyramid_height_[l]));
                        pkf->gaus_pyramid_original_image_[l] =
                                tensor_utils::cvMat2TorchTensor_Float32(img_resized, device_type_);

                        cv::cuda::resize(pkf->img_auxiliary_undist_, depth_resized,
                                         cv::Size(pkf->gaus_pyramid_width_[l], pkf->gaus_pyramid_height_[l]));
                        pkf->gaus_pyramid_original_depth_[l] =
                                tensor_utils::cvMat2TorchTensor_Float32(depth_resized, device_type_);
                    }
                }
            }
    ...
    while (getIteration() - SLAM_stop_iter <= n_delay_iters || getIteration() % densify_interval <= n_delay_iters ||
           isKeepingTraining()) {
        trainForOneIteration();
    ...
    // Save and clear
    renderAndRecordAllKeyframes("experiment");
    savePly(result_dir_ / "experiment" / "ply");
    writeKeyframeUsedTimes(result_dir_ / "experiment" / "used_times", "final");
...
void GaussianMapper::trainForOneIteration() {
    ...
    torch::Tensor gt_image, mask, gt_depth;
    ...
    if (training_level == num_gaus_pyramid_sub_levels_) {
        ...
        gt_depth = viewpoint_cam->gaus_pyramid_original_depth_[training_level].cuda();
    }
    ...
    gaussians_->setLanguageFeatureLearningRate(languageFeatureLearningRate());
    ...
    auto rendered_lf = std::get<1>(render_pkg);
    auto rendered_depth = std::get<2>(render_pkg);
    auto viewspace_point_tensor = std::get<3>(render_pkg);
    auto visibility_filter = std::get<4>(render_pkg);
    auto radii = std::get<5>(render_pkg);

    // TODO: submit for 'if'
    auto gt_lf = torch::squeeze(torch::nn::functional::interpolate(torch::unsqueeze(viewpoint_cam->language_features_, 0),
                                                    torch::nn::functional::InterpolateFuncOptions().size(std::vector({rendered_lf.size(1), rendered_lf.size(2)}))));

    // Get rid of black edges caused by undistortion
    torch::Tensor masked_image = rendered_image * mask;
    torch::Tensor masked_lf = rendered_lf * torch::unsqueeze(mask[0], 0);  // TODO: Is it necessary?
    torch::Tensor masked_depth = rendered_depth * torch::unsqueeze(mask[0], 0);  // TODO: Is it necessary?

    // Loss
    auto Ll1 = loss_utils::l1_loss(masked_image, gt_image);
    auto similarity_lf = loss_utils::cosine_similarity(masked_lf, gt_lf);
    auto Ll1_depth = loss_utils::l1_loss(masked_depth, gt_depth);
    float lambda_dssim = lambdaDssim();
    auto loss = (1.0 - lambda_dssim) * Ll1
                + lambda_dssim * (1.0 - loss_utils::ssim(masked_image, gt_image, device_type_)) + similarity_lf + Ll1_depth;

//    auto l1 = gaussians_->language_features_.size(0), s1 = gaussians_->features_dc_.size(0);
    loss.backward();
...
// renderAndRecordKeyframe(pkf, result_dir_, "_0_before_loop_correction");
                        ...
                        if (large_rot || large_trans) {
                            std::cout
                                    << "[Gaussian Mapper]Large loop correction detected, transforming visible points of kf "
                                    << kfid << std::endl;
...
void GaussianMapper::handleNewKeyframe(
    ...
    try {
        ...
        // LF Auxiliary Image
        imgAuxLF_undistorted = std::get<6>(kf);

        pkf->original_image_ =
                tensor_utils::cvMat2TorchTensor_Float32(imgRGB_undistorted, device_type_);
        pkf->language_features_ =
                tensor_utils::cvMat2TorchTensor_Float32(imgAuxLF_undistorted, device_type_);
        pkf->original_depth_ =
                tensor_utils::cvMat2TorchTensor_Float32(imgAux_undistorted, device_type_);
        pkf->img_filename_ = std::get<9>(kf);
    ...
    pkf->img_auxiliary_lf_undist_ = imgAuxLF_undistorted;
    pkf->kps_pixel_ = std::move(std::get<7>(kf));
    pkf->kps_point_local_ = std::move(std::get<8>(kf));
...
void GaussianMapper::recordKeyframeRendered(
    ...
    if (record_rendered_image_) {
        ...
        cv::imwrite(
                result_img_dir / (std::to_string(getIteration()) + "_" + std::to_string(kfid) + name_suffix + ".jpg"),
                image_cv);
    }
    ...
    if (record_ground_truth_image_) {
        ...
        cv::imwrite(
                result_gt_dir / (std::to_string(getIteration()) + "_" + std::to_string(kfid) + name_suffix + "_gt.jpg"),
                gt_image_cv);
    }
    ...
    if (record_loss_image_) {
        ...
        cv::imwrite(result_loss_dir /
                    (std::to_string(getIteration()) + "_" + std::to_string(kfid) + name_suffix + "_loss.jpg"),
                    loss_image_cv);
    }
}
...
cv::Mat GaussianMapper::renderFromPose(
    ...
    std::tuple <at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> render_pkg;
    {
        std::unique_lock <std::mutex> lock_render(mutex_render_);
        // Render
...
void GaussianMapper::renderAndRecordKeyframe(
    ...
    recordKeyframeRendered(masked_image, gt_image, pkf->fid_, result_img_dir, result_gt_dir, result_loss_dir,
                           name_suffix);
}

void GaussianMapper::renderAndRecordAllKeyframes(
        std::string exp_name) {
    std::filesystem::path result_dir = result_dir_ / exp_name;
    CHECK_DIRECTORY_AND_CREATE_IF_NOT_EXISTS(result_dir)
...
float GaussianMapper::languageFeatureLearningRate() {
    std::unique_lock <std::mutex> lock(mutex_settings_);
    return opt_params_.language_feature_lr_;
}
