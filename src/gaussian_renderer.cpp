...
GaussianRenderer::render(
        ...
        bool include_language_features) {
    /* Render the scene. 
    ...
    bool has_lang_feat = false;
    torch::Tensor lang_feat;
    if (include_language_features) {
        has_lang_feat = true;
        lang_feat = pc->getLanguageFeatures();
    }
    ...
    auto rendered_lf = std::get<1>(rasterizer_result);
