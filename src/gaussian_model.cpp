...
torch::Tensor GaussianModel::getLanguageFeatures() {
    return this->language_features_.clone();
}
...
torch::Tensor GaussianModel::getCovarianceActivation(int scaling_modifier) {
    ...
    // strip_symmetric
    // strip_lowerdiag
    auto symm_uncertainty = torch::zeros({actual_covariance.size(0), 6},
...
void GaussianModel::createFromPcd(
    ...
    torch::Tensor color = torch::zeros(
            {num_points, 3},
            torch::TensorOptions().dtype(torch::kFloat).device(device_type_));
    torch::Tensor lang_feat = torch::zeros(
            {num_points, LANGUAGE_FEATURES_DIM},
            torch::TensorOptions().dtype(torch::kFloat).device(device_type_));
    ...
    for (int point_idx = 0; point_idx < num_points; ++point_idx) {
        ...
        for (int lf_idx = 0; lf_idx < LANGUAGE_FEATURES_DIM; ++lf_idx) {
            lang_feat.index({point_idx, lf_idx}) = point.lang_features_(lf_idx);
        }
        ++pcd_it;
    }
    ...
    this->features_rest_ = features.index({torch::indexing::Slice(),
            ...
            .contiguous()
            .requires_grad_();
    this->language_features_ = lang_feat.requires_grad_();
...
// auto time1 = std::chrono::steady_clock::now();
    ...
    torch::Tensor new_lang_feat = torch::zeros(
            {num_new_points, LANGUAGE_FEATURES_DIM},
            torch::TensorOptions().dtype(torch::kFloat).device(device_type_));
    ...
    auto new_features_rest = features.index({torch::indexing::Slice(),
            ...
            .transpose(1, 2)
            .contiguous();
    auto new_language_features = new_lang_feat.contiguous();
...
// auto time1 = std::chrono::steady_clock::now();
    ...
    features.index(
            {torch::indexing::Slice(),
             torch::indexing::Slice(3, features.size(1)),
             torch::indexing::Slice(1, features.size(2))}) = 0.0f;
    torch::Tensor language_features = torch::zeros(
            {new_fused_colors.size(0), LANGUAGE_FEATURES_DIM},
            torch::TensorOptions().dtype(torch::kFloat).device(device_type_));
    // std::cout << "[Gaussian Model]Number of points increase : "
    //           << num_new_points << std::endl;
    ...
    auto new_features_rest = features.index({torch::indexing::Slice(),
            ...
            .transpose(1, 2)
            .contiguous();
    auto new_language_features = language_features.contiguous();
    auto new_opacities = opacities;
    auto new_scaling = scales;
    auto new_rotation = rots;
...
void GaussianModel::scaledTransformationPostfix(
    ...
    torch::Tensor optimizable_scaling = this->replaceTensorToOptimizer(new_scaling, 5);
...
// scales = scales.unsqueeze(scales_ndimension).repeat({1, 3});
    ...
    // param_groups[3] = language_feature_
    // param_groups[4] = opacity_
    // param_groups[5] = scaling_
    // param_groups[6] = rotation_
    // ==================================
    ...
    torch::Tensor optimizable_rots = this->replaceTensorToOptimizer(rots, 6);
...
void GaussianModel::trainingSetup(const GaussianOptimizationParams &training_args) {
    ...
    optimizer_->add_param_group(Tensor_vec_language_feature_);
    optimizer_->param_groups()[3].options().set_lr(training_args.language_feature_lr_);

    optimizer_->add_param_group(Tensor_vec_opacity_);
    optimizer_->param_groups()[4].options().set_lr(training_args.opacity_lr_);

    optimizer_->add_param_group(Tensor_vec_scaling_);
    optimizer_->param_groups()[5].options().set_lr(training_args.scaling_lr_);

    optimizer_->add_param_group(Tensor_vec_rotation_);
    optimizer_->param_groups()[6].options().set_lr(training_args.rotation_lr_);
...
void GaussianModel::setLanguageFeatureLearningRate(float lang_feat_lr) {
    optimizer_->param_groups()[3].options().set_lr(lang_feat_lr);
}

void GaussianModel::setOpacityLearningRate(float opacity_lr) {
    optimizer_->param_groups()[4].options().set_lr(opacity_lr);
}

void GaussianModel::setScalingLearningRate(float scaling_lr) {
    optimizer_->param_groups()[5].options().set_lr(scaling_lr);
}

void GaussianModel::setRotationLearningRate(float rot_lr) {
    optimizer_->param_groups()[6].options().set_lr(rot_lr);
}
...
void GaussianModel::resetOpacity() {
    ...
    torch::Tensor optimizable_tensors = this->replaceTensorToOptimizer(opacities_new, 4); // "opacity"
...
void GaussianModel::prunePoints(torch::Tensor &mask) {
    ...
    // _prune_optimizer
    std::vector <torch::Tensor> optimizable_tensors(7);
    auto &param_groups = this->optimizer_->param_groups();
    auto &state = this->optimizer_->state();
    for (int group_idx = 0; group_idx < 7; ++group_idx) {
    ...
    // param_groups[1] = feature_dc_
    // param_groups[2] = feature_rest_
    // param_groups[3] = language_features_
    // param_groups[4] = opacity_
    // param_groups[5] = scaling_
    // param_groups[6] = rotation_
    // ==================================
    ...
    this->language_features_ = optimizable_tensors[3];
    this->opacity_ = optimizable_tensors[4];
    this->scaling_ = optimizable_tensors[5];
    this->rotation_ = optimizable_tensors[6];
...
void GaussianModel::densificationPostfix(
        ...
        torch::Tensor &new_language_features,
    ...
    for (int group_idx = 0; group_idx < 7; ++group_idx) {
        ...
        if (state.find(key) != state.end()) {
            ...
            new_state->step(stored_state.step());
            new_state->exp_avg(
                    torch::cat({stored_state.exp_avg().clone(), torch::zeros_like(extension_tensor)}, /*dim=*/0));
            new_state->exp_avg_sq(
                    torch::cat({stored_state.exp_avg_sq().clone(), torch::zeros_like(extension_tensor)}, /*dim=*/0));
            // new_state->max_exp_avg_sq(stored_state.max_exp_avg_sq().clone());  // needed only when options.amsgrad(true), which is false by default
...
void GaussianModel::densifyAndSplit(
    ...
    auto new_xyz = torch::bmm(rots, samples.unsqueeze(-1)).squeeze(-1) +
                   this->getXYZ().index({selected_pts_mask}).repeat({N, 1});
    auto new_scaling = torch::log(this->getScalingActivation().index({selected_pts_mask}).repeat({N, 1}) /
    ...
    auto new_language_features = this->language_features_.index({selected_pts_mask}).repeat({N, 1});
    ...
    auto prune_filter = torch::cat({
                                           selected_pts_mask,
                                           torch::zeros({(N * selected_pts_mask.sum()).item<int>()},
...
void GaussianModel::densifyAndClone(
    ...
    auto new_language_features = this->language_features_.index({selected_pts_mask});
...
void GaussianModel::savePly(std::filesystem::path result_path) {
    ...
    torch::Tensor lf = this->language_features_.detach().cpu();
    ...
    // lf
    std::size_t n_lf = this->language_features_.size(1);
    std::vector <std::string> property_names_lf(n_lf);
    for (int i = 0; i < n_lf; ++i)
        property_names_lf[i] = "lf_" + std::to_string(i);

    result_file.add_properties_to_element(
            "vertex", property_names_lf,
            tinyply::Type::FLOAT32, this->language_features_.size(0),
            reinterpret_cast<uint8_t *>(lf.data_ptr<float>()),
            tinyply::Type::INVALID, 0);
...
float GaussianModel::exponLrFunc(int step) {
    ...
    if (lr_delay_steps_ > 0)
        delay_rate = lr_delay_mult_ + (1.0f - lr_delay_mult_) * std::sin(
                M_PI_2f32 * std::clamp(static_cast<float>(step) / lr_delay_steps_, 0.0f, 1.0f));
    else
        delay_rate = 1.0f;
