...
namespace ORB_SLAM3 {
    ...
    Map::Map() : mnMaxKFid(0), mnBigChangeIdx(0), mbImuInitialized(false), mnMapChange(0),
                 ...
                 mbFail(false), mIsInUse(false), mbBad(false), mnMapChangeNotified(0), mbIsInertial(false),
...
//        delete *sit;
    ...
    void Map::PreSave(std::set<GeometricCamera *> &spCams) {
        ...
        for (MapPoint *pMPi: mspMapPoints) {
            ...
            for (map < KeyFrame * , std::tuple < int, int >> ::iterator it = mpObs.begin(), end = mpObs.end(); it !=
                                                                                                               end;
            ++it)
            {
