...
Sophus::SE3f Tracking::GrabImageRGBDLF(const cv::Mat &imRGB,const cv::Mat &imD, const cv::Mat &imLF, const double &timestamp, string filename)
    {
        mImGray = imRGB;
        cv::Mat imDepth = imD;
        ...
        if (mSensor == System::RGBD)
            mCurrentFrame = Frame(mImGray,imDepth, mImRGB, imLF, timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
        else if(mSensor == System::IMU_RGBD)
            mCurrentFrame = Frame(mImGray,imDepth,mImRGB, imLF, timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
