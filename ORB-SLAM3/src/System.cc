...
namespace ORB_SLAM3 {
    ...
    Sophus::SE3f System::TrackRGBDLF(const cv::Mat &im, const cv::Mat &depthmap, const cv::Mat &lfmap, const double &timestamp,
                                     const vector <IMU::Point> &vImuMeas, string filename) {
        if (mSensor != RGBD && mSensor != IMU_RGBD) {
            cerr << "ERROR: you called TrackRGBDLF but input sensor was not set to RGBD." << endl;
            exit(-1);
        }

        cv::Mat imToFeed = preprocessImage(im);
        cv::Mat imDepthToFeed = depthmap.clone();
        cv::Mat imLFToFeed = lfmap.clone();
        if (settings_ && settings_->needToResize()) {
            cv::Mat resizedIm;
            cv::resize(im, resizedIm, settings_->newImSize());
            imToFeed = resizedIm;
        ...
        Sophus::SE3f Tcw = mpTracker->GrabImageRGBDLF(imToFeed, imDepthToFeed, imLFToFeed, timestamp, filename);
