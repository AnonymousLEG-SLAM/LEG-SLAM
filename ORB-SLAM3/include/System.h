...
public:
    ...
    // Process the given rgbdlf frame. Depthmap must be registered to the RGB frame.
    // Input image: RGB (CV_8UC3) or grayscale (CV_8U). RGB is converted to grayscale.
    // Input depthmap: Float (CV_32F).
    // Input lfmap: Float(CV_32F)
    // Returns the camera pose (empty if tracking fails).
    Sophus::SE3f TrackRGBDLF(const cv::Mat &im, const cv::Mat &depthmap, const cv::Mat &lfmap, const double &timestamp, const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="");
