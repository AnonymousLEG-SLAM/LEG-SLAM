...
namespace loss_utils {
    ...
    inline torch::Tensor cosine_similarity(torch::Tensor &network_output, torch::Tensor &gt) {
        return torch::nn::functional::cosine_similarity(network_output.view({LANGUAGE_FEATURES_DIM, -1}),
                                                        gt.view({LANGUAGE_FEATURES_DIM, -1}),
                                                        torch::nn::CosineSimilarityOptions().dim(0)).mean();
    }
