 ...
 */
    inline torch::Tensor cvMat2TorchTensor_Float32(
        ...
        if (mat.channels() == 1) {
            ...
            tensor = mat_tensor.clone().to(device_type).permute({2, 0, 1});
        }
    ...
    inline void normalize(const cv::Mat &src, cv::Mat &dst, const cv::Vec3f &mean, const cv::Vec3f &std) {
        for (int idx = 0; idx < src.rows; ++idx) {
            for (int jdx = 0; jdx < src.cols; ++jdx) {
                for (int kdx = 0; kdx < src.channels(); ++kdx) {
                    dst.at<cv::Vec3f>(idx, jdx)[kdx] = (dst.at<cv::Vec3f>(idx, jdx)[kdx] - mean[kdx]) / std[kdx];
                }
            }
        }
    }
