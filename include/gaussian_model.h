...
#define GAUSSIAN_MODEL_TENSORS_TO_VEC                                \
    ...
    this->Tensor_vec_language_feature_ = {this->language_features_}; \
...
#define GAUSSIAN_MODEL_INIT_TENSORS(device_type)                                             \
    ...
    this->language_features_ = torch::empty(0, torch::TensorOptions().device(device_type));  \
...
public:
    ...
    torch::Tensor getLanguageFeatures();
    ...
    void setLanguageFeatureLearningRate(float lang_feat_lr);
...
public:
    ...
    torch::Tensor language_features_;
